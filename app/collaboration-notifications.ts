...
export const workflowEventLabels: Record<string, string> = { claimed: "安排认领", submit: "提交完成", approve: "审批通过", reject: "打回修改", completed: "完成同步", "owner-complete": "发布者直接完成", updating: "修改详情", updated: "详情已同步", "update-replaced": "详情修改已替代", "task-anomaly": "任务状态异常", "task-relocated": "关联位置已更新", "restore-requested": "恢复任务", "tasks-restored": "任务已恢复", nudge: "催办", "reply-nudge": "回复催办" };
function available(source: NoticeSource): TaskNotice[] {
  ...
  for (const workflow of Object.values(source.workflows)) {
    ...
    for (const event of workflow.events) items.push({ id: `workflow:${workflow.id}:${event.id}`, workflowId: workflow.id, eventId: event.id, eventType: event.type, kind: "workflow", title: workflow.title, body: `${workflowEventLabels[event.type] || "流程更新"}${event.comment ? `：${event.comment.slice(0, 160)}` : ""}`, at: event.at, actorId: event.actorId, recipients: [...new Set([workflow.reviewerId, workflow.claimantId])] });
  }
  return items;
}
...
export function unreadTaskNotices(source: NoticeSource, actor: string): TaskNotice[] {
  ...
  return notices.entries.filter(item => receivesTaskNotice(item, actor) && !seen.has(item.id) && (item.kind !== "public" || (source.buffer[item.taskId!] && !source.buffer[item.taskId!].completedAt)));
}
