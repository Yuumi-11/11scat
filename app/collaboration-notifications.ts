...
function available(source: NoticeSource): TaskNotice[] {
  ...
  for (const workflow of Object.values(source.workflows)) {
    ...
    for (const event of workflow.events.filter(event => event.type !== "completed" && !silentWorkflowEvent(event.type))) items.push({ id: `workflow:${workflow.id}:${event.id}`, workflowId: workflow.id, eventId: event.id, eventType: event.type, kind: "workflow", title: workflow.title, body: `${workflowEventLabels[event.type] || "流程更新"}${event.comment ? `：${event.comment.slice(0, 160)}` : ""}`, at: event.at, actorId: event.actorId, recipients: [...new Set([workflow.reviewerId, workflow.claimantId])] });
  }
  return items;
}
