...
export async function POST(request: NextRequest) {
  ...
  if (!body || typeof body.attachmentId !== "string" || !/^[0-9a-f-]{36}$/i.test(body.attachmentId)) {
    return NextResponse.json({ error: "附件不存在" }, { status: 400 });
  }
  try {
    const item = await readChatAttachmentMetadata(body.attachmentId);
    const path = await importChatAttachment(body.attachmentId, item.sourcePath, item.name, item.size, item.kind === "audio" ? "video" : "chat/pics");
    return NextResponse.json({ path }, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: error instanceof CloudCapacityError ? error.message : "附件保存到云盘失败" }, { status: error instanceof CloudCapacityError ? 507 : 404 });
  }
}
