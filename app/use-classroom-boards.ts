...
import { addAndSelectBoard, replaceClassroomBoards, type ClassroomBoards } from './classroom-boards';
...
export function useClassroomBoards() {
  ...
  const setBoards = useCallback((update: SetStateAction<RoomBoard[]>) => setState(current => replaceClassroomBoards(current, typeof update === 'function' ? update(current.boards) : update)), []);
