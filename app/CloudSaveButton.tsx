...
export function CloudSaveButton({ state, onClick }: { state?: CloudSaveState; onClick: () => void }) {
  ...
  return <button className={`image-cloud-button cloud-save-icon ${state || ""}`} type="button" onClick={onClick} disabled={state === "uploading" || state === "done"} title={label} aria-label={label} aria-busy={state === "uploading"}><Icon size={18} aria-hidden="true" /></button>;
}
