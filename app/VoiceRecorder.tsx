...
export function VoiceRecorder({ onRecorded, onError }: { onRecorded: (file: File) => void; onError: (message: string) => void }) {
  ...
  return active ? <div className="voice-recording" role="group" aria-label="正在录音">
    ...
    <button type="button" disabled={state.phase === "finishing"} onClick={() => session.current?.stop()}  aria-label="停止并发送语音"><Square size={18} /></button>
    <button type="button" disabled={state.phase === "finishing"} onClick={() => session.current?.stop(true)}  aria-label="取消录音，不发送"><X size={18} /></button>
  </div> : <button className="chat-voice-button" type="button" onClick={() => void session.current?.start()} disabled={state.phase === "requesting"}  aria-label="录制语音"><Mic size={20} aria-hidden="true" /></button>;
}
