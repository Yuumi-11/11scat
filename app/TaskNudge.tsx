...
export function TaskNudge({ notice, onRead, onClose }: { notice: TaskNotice; onRead: (ids: string[]) => Promise<void>; onClose: () => void }) {
  ...
  return createPortal(<dialog ref={dialog} className="task-nudge-dialog" aria-labelledby="task-nudge-title" onCancel={event => { event.preventDefault(); if (!busy) void dismiss(); }} onKeyDown={event => event.stopPropagation()}>
    <header><BellRing size={23} /><h3 id="task-nudge-title">同桌催办了这项任务</h3><button type="button" className="coop-icon" aria-label="稍后回复" disabled={busy} onClick={() => void dismiss()}><X size={20} /></button></header>
    ...
    }}><label htmlFor="task-nudge-reply">回复任务发起人</label><textarea id="task-nudge-reply" rows={3} maxLength={2000} value={reply} disabled={busy} onChange={event => { setReply(event.target.value); commandId.current = crypto.randomUUID(); }} placeholder="告诉同桌当前进展或预计完成时间" />
      {error && <p className="coop-feedback error" role="alert">{error}</p>}
      <footer><button type="button" disabled={busy} onClick={() => void dismiss()}>稍后回复</button><button type="submit" disabled={busy || !reply.trim()}><Send size={16} />发送回复</button></footer>
    </form>
