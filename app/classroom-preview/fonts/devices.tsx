...
const choices = [
  ...
  {id:'youyuan',name:'幼圆 + Nunito · 当前使用',note:'英文与数字使用本地 Nunito 圆体；中文使用系统幼圆，缺失时由 Noto Sans SC 显示。'},
];
export function DeviceFontComparison() {
  ...
  return <main className="calendar-font-review">
    ...
    <section className="calendar-font-grid classroom-scene">{choices.map(choice=><article key={choice.id} className="calendar-font-option device-font-option classroom-scene" data-device-font={choice.id}>
      <h2>{choice.name}</h2><DeviceCard kind="tablet" name={name} online screen camera self={false}><p>{activity}</p></DeviceCard><DeviceCard kind="laptop" name={name} online screen={false} camera={false} self={false}><p>{activity}</p></DeviceCard><p>{choice.note}</p><a href={'/classroom-preview?font='+choice.id}>在教室里查看</a>
    </article>)}</section>
  </main>;
}
