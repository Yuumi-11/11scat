...
export function ClassroomPreview({ deviceFont = 'youyuan' }: { deviceFont?: string }) {
  ...
  return <main className="app-shell classroom-scene" data-device-font={profile.font} aria-label="本地教室外观预览">
    <section className="workspace"><section className="focus-stage panel"><div className="share-canvas" ref={stageRef}><div className="stage-content">
      ...
      <aside className="side-panel panel"><div className="side-tabs"><button className={side==='chat'?'active':''} onClick={()=>setSide('chat')}>传纸条</button><button className={side==='tasks'?'active':''} onClick={()=>setSide('tasks')}>今日todo</button><div className="chat-bell-host"><button className="room-bell-trigger" aria-label="摇铃"><ClassroomProp name="bell" /></button></div></div>
    ...
    <div className="classroom-desk desk-media"><button className="object-button" aria-label="画板" onClick={createBoard}><ClassroomProp name="chalk-cup"/></button><button className="object-button calendar-entry-button" aria-label="双人日历"  onClick={()=>setNotice('双人日历将在后续开放')}><ClassroomProp name="calendar-entry"/></button></div>
