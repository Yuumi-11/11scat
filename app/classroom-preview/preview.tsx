...
export function ClassroomPreview({ initialBoardTone = 'deep' }: { initialBoardTone?: string }) {
  ...
  return <main className="app-shell classroom-scene" data-device-font={profile.font} style={boardToneStyle(boardTone)} aria-label="本地教室外观预览">
    <section className="workspace"><section className="focus-stage panel"><div className="share-canvas" ref={stageRef}><div className="stage-content">
      ...
      <aside className="side-panel panel"><div className="side-tabs"><button className={side==='chat'?'active':''} onClick={()=>setSide('chat')}>传纸条</button><button className={side==='tasks'?'active':''} onClick={()=>setSide('tasks')}>今日任务</button><div className="chat-bell-host"><button className="room-bell-trigger" aria-label="摇铃"><ClassroomProp name="bell" /></button></div></div>
    ...
    <div className="classroom-desk desk-room"><button className="object-button cloud-entry-button" aria-label="云盘"><ClassroomProp name="folder"/></button><ClassroomSettings key={settingsIdentity} profile={profile} identityId={settingsIdentity} onSave={saveSettings} triggerContent={<ClassroomProp name="settings"/>} /></div></div>
    <EmergencyExit/>
