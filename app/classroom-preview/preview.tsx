...
import { Check, Paperclip, Mic, Send, Expand, LogOut } from 'lucide-react';
...
export function ClassroomPreview() {
  ...
  return <main className="app-shell classroom-scene" aria-label="本地教室外观预览">
    <section className="workspace">
      <section className="focus-stage panel"><div className="share-canvas" ref={stageRef}>
        <button className="object-button projector-control" type="button" aria-label="共享屏幕" onClick={()=>setMode(mode==='projection'?'idle':'projection')}><ClassroomProp name="projector" /></button>
        {mode==='idle' && <IdleChalkboard date={date} tasks={previewTasks} />}
        <div className={mode==='projection'?'projection-sheet is-open':'projection-sheet'} aria-hidden={mode!=='projection'}>{mode==='projection' && <div className="preview-slide"><strong>{previewTasks[0].title}</strong>{previewTasks.slice(1).map(t=><div key={t.id}>{t.title}</div>)}</div>}</div>
        ...
        {mode!=='board' && <button className="main-fullscreen-button" aria-label="主窗口全屏" onClick={()=>void toggleFullscreen()}><Expand /></button>}
    ...
    <div className="scene-desks"><div className="classroom-desk"><CalendarCard name="11"><form className="activity-box" onSubmit={e=>e.preventDefault()}><input aria-label="我正在" defaultValue="复习数据结构" maxLength={80}/></form></CalendarCard></div><div className="classroom-desk"><CalendarCard name="11scat"><p>整理课堂笔记</p></CalendarCard></div><div className="classroom-desk desk-media">{(['chalk-cup','microphone','camera'] as const).map((name,i)=><button className="object-button" key={name} aria-label={['画板','麦克风','摄像头'][i]}><ClassroomProp name={name}/></button>)}<button className="wall-exit" aria-label="退出"><LogOut size={19}/>退出</button></div><div className="classroom-desk desk-room">{(['taskboard','folder','settings'] as const).map((name,i)=><button className="object-button" key={name} aria-label={['任务板','云盘','设置'][i]}><ClassroomProp name={name}/></button>)}</div></div>
    <div className="classroom-preview-toolbar" aria-label="本地预览工具"><span>本地预览</span>{[['idle','待机'],['projection','投影'],['board','画板']].map(([value,label])=><button key={value} onClick={()=>setMode(value)} aria-pressed={mode===value}>{label}</button>)}<button onClick={()=>{const next=normalizeBoard(JSON.parse(JSON.stringify(board)));if(next)setBoard(next);setNotice('已重新载入');}}>重新载入笔迹</button>{mode==='board' && <span>{board.strokes.length} 笔 · {board.strokes.at(-1)?.points.length || 0} 点</span>}{notice && <span role="status">{notice}</span>}</div>
  </main>;
}
