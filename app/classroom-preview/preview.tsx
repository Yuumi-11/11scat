  ...
  return <main className="app-shell classroom-scene" aria-label="本地教室外观预览">
    <section className="workspace">
      <section className="focus-stage panel"><div className="share-canvas" ref={stageRef}>
        ...
        {mode!=='board' && !projection.open && <IdleChalkboard date={date} tasks={previewTasks} />}
        <div className={projection.open?'projection-sheet is-open':'projection-sheet'} aria-hidden={!projection.open}>{projection.open && <div className="preview-slide"><strong>{previewTasks[0].title}</strong>{previewTasks.slice(1).map(t=><div key={t.id}>{t.title}</div>)}</div>}</div>
