...
恢复不会改变流程状态。待审批仍为待审批，原提交附件、完成说明与此前的审批评语全部保留。按 2026-09-10 最新要求，关联任务在提交后变化不再阻止审批，恢复后也可直接通过。已进入完成同步的流程恢复任务后不会自动继续勾选，需要按现有重试入口继续。
...
恢复失败会显示原因并保留进度，自动重试间隔至少一分钟，参与者可以点击“重试同步”。响应丢失后若读到原任务已经恢复且配置一致，直接确认，不再次写入。恢复操作核对刚执行的状态变更是否成功；按 2026-09-10 最新要求，原任务方或认领者在提交后修改任务不再阻止审批，需要补充成果时由审批人明确打回。
