...
各成员真实收集箱编号通过官方 `GET /open/v1/project/inbox/data` 返回的 `project.id` 获取，任务板与协作区共用同一读取函数。`inbox` 由当前请求的 Bearer 授权确定所属账户，不依赖 V2 账户快照或全局收集箱配置；后续更新、删除和分配继续使用返回的真实编号核对归属。空收集箱也从 `project.id` 识别，不能根据其他清单的任务猜测编号。
