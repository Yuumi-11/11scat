...
![自习室协作区新版静态效果示意](images/room-collaboration-layout-2026-09-08.png)

上图使用项目现有图标及雾蓝主题方向绘制，成员和任务均为虚构示例，属于静态效果示意。它展示本次布局与操作方向，不代表真实浏览器截图或交互验收结果；左侧第二张个人任务卡展示标题输入状态，实际拖放通过整个目标区域高亮提示落点。

协作弹窗上方是横跨整行的公共待分配区，名称为“任务板”，约占内容区高度的三成；下方两位成员的收集箱左右均分。全部已登记成员都会显示，包括当前离线成员；超过两人时以两列继续排列，窄屏保留成员列的可读宽度并允许横向滚动。分列只表示任务归属，不引入按状态划分的看板，也不调用滴答官方共享清单协作。未连接滴答的成员仍可使用公共任务板，其成员列会提示连接状态，在接入前不能接收分配到个人收集箱的任务。
...
本次保留缩小后的“自习室协作”标题，删除 `ROOM TASKS`、标题下的操作说明、成员收集箱副标题与空白区说明，也删除常驻底部操作记录。无日期任务不再显示“无日期”，保存结果短暂显示，错误及未完成操作的恢复入口继续保留。

| 用户本轮要求 | 对应结果 |
| --- | --- |
| 上方完整待分配区，下方左右均分 | 公共任务板独占上方，成员区使用等宽双列，各列表独立滚动。 |
| 移除分配表单 | 移除卡片成员下拉框，保留认领及拖动，握柄同时可用键盘操作。 |
| 编辑不单占一行 | 标题原位输入，详情入口缩为省略号图标。 |
| 清除冗余说明 | 按截图去掉重复说明，保留紧凑弹窗名称和必要的操作反馈。 |
| 沿用云盘新增逻辑 | 末尾虚线加号卡片点击后直接命名。 |
...
初版通过生产构建、TypeScript 检查及当时的 31 项自动化测试；本轮布局调整通过生产构建、TypeScript、相关组件 ESLint 与当前全量 39 项测试。协作测试使用独立临时数据与模拟滴答接口，覆盖 HTTP 身份与来源检查、跨成员缓冲区可见性、过期版本拒绝，以及多人认领和转移失败恢复；提供方适配测试还验证授权账户与收集箱匹配、固定 ID 重试和已有检查项等字段保留。没有为测试修改任何真实滴答账户，这些测试不能代替新增输入框的浏览器事件验收。
